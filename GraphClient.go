...
// Package  itswizard_azureactivedirctory075 is a go lang implementation of the Microsoft Graph API
//
...
func (g *GraphClient) makeGETAPICall(apicall string, getParams url.Values, v interface{}) error {
	...
	getParams.Add("$top", strconv.Itoa(100))
...
func (g *GraphClient) ListUsers() (Users, error) {
	...
	if len(output) == 999 {
		// Part for Paging
		// func paging(nextlink string,
		link := marsh.NextLink
		for {
			g.makeGETNext(link, &marsh)
			for i := 0; i < len(marsh.Users); i++ {
				output = append(output, marsh.Users[i])
			}
			if link == marsh.NextLink {
				break
			} else {
				link = marsh.NextLink
			}
...
func (g *GraphClient) GetMemberIdsOfGroup(id string) (usersIds []string, err error) {
	t := make(map[string]User)
	resource := "/groups/" + id + "/members"
	err = g.makeGETAPICall(resource, nil, &marsh)
	for i := 0; i < len(marsh.Users); i++ {
		t[marsh.Users[i].ID] = marsh.Users[i]
	}
	link := marsh.NextLink

	if link != "" {
		for len(marsh.Users) > 0 {
...
func (g *GraphClient) GetMembersOfGroup(id string) (users []msgraph.User, err error) {
	t := make(map[string]User)
	aad2 := itswizard_m_msgraph.NewAADAction(g.TenantID, g.ApplicationID, g.ClientSecret)
	resource := "/groups/" + id + "/members"
	err = g.makeGETAPICall(resource, nil, &marsh)
	for i := 0; i < len(marsh.Users); i++ {
		t[marsh.Users[i].ID] = marsh.Users[i]
...
// makeGETNext performs an API-Call to the msgraph API. This func uses sync.Mutex to synchronize all API-calls
// This call is specially for Paging
func (g *GraphClient) makeGETNext(call string, v interface{}) error {
	g.apiCall.Lock()
	defer g.apiCall.Unlock() // unlock when the func returns
	// Check token
	if g.token.WantsToBeRefreshed() { // Token not valid anymore?
	...
	reqURL, err := url.ParseRequestURI(call)
	if err != nil {
		return fmt.Errorf("Unable to parse call %v: %v", call, err)
	}
