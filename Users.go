...
func (u *User) AddAddress() error {
	...
	err := u.graphClient.makeGETNext(fmt.Sprint(BaseURL, "/", APIVersion, "/", "users", "/", u.ID, "?$select=state,city,postalcode,streetAddress,country"), &marsh)
	if err != nil {
		return err
	}
	u.State = marsh.State
	u.PostalCode = marsh.PostalCode
	u.City = marsh.City
	u.StreetAddress = marsh.StreetAddress
	u.Country = marsh.Country
	return nil
}
